...
const Backlink = ({ backlinks }) => {
    ...
    return (
        <>
            ...
            <div className="bg-white h-auto text-white w-full rounded-b-lg rounded-tl-lg text-end p-8">
                <div className="flex mt-4">
                    {backlinks.map((item, i) => (
                        item.status == 0 ?
                        <button onClick={() => handleclick(item.id)} key={i}>
                        ...
                        :
                        <ProgressList datetime={item.datetime} key={i}/>                        
                    ))}
                </div>
            </div>
